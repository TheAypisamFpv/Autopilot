    ...
    with torch.no_grad():
        while cap.isOpened():
            ...
            if len(frameBuffer) >= frameBufferSize:
                ...
                dynamicData = torch.tensor([[0.0, 0.0, 0.0]], dtype=torch.float32).to(device)
            ...
            # --- Real-time playback and exit condition ---
            processingTime = time.time() - loopStartTime
            waitTime = (timePerFrame - processingTime) * 1000
            key = cv2.waitKey(max(1, int(waitTime))) if waitTime > 0 else cv2.waitKey(1)
...
if __name__ == '__main__':
    modelPath = r"F:\VS_Python_Project\Autopilot\Autopilot\training\run9\best_model.pth"
