...
def detect_lane_width(frame, displayed_frame):
    ...
    return displayed_frame, masked_frame, left_line_x_pos, left_found, right_line_x_pos, right_found, true_lane_width
...
def main():
    ...
    Video here
    """
    cap = cv.VideoCapture('Test drive\\23_03_2023_01.MP4')
    while (cap.isOpened()):
        ...
        if ret:
            ...
            print(f"left_line_incertitude: {int(left_line_incertitude): =04d}  right_line_incertitude: {int(right_line_incertitude): =04d}  lane width: {true_lane_width: =04d}   average lane width: {int(average_lane_width): =04d}  left_found: {int(left_found)}  right_found: {int(right_found)}")
