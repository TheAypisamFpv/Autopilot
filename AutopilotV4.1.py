...
def detect_lines(frame, prev_left_line_x_pos, prev_right_line_x_pos):
    ...
    left_base = np.argmax(
        histogram[exterior_threshold:lane_center - interior_threshold]) + exterior_threshold
    right_base = np.argmax(histogram[lane_center + interior_threshold:width -
                           exterior_threshold]) + lane_center + interior_threshold
    ...
    print(left_boxs_x[0], left_boxs_y[0])
...
def main():
    ...
    Video here
    """
    cap = cv.VideoCapture('Test drive\\30 03 2023\\2.MP4')
